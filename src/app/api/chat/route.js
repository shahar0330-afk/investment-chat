...
11. **🔴 תמונת נכסים — כללים קריטיים!**
    **מתי להשתמש ב-update_portfolio:**
    - כשהלקוח אומר במפורש שיש לו נכס עם סכום
    ...
    **מתי אסור:**
    - ❌ להוסיף נכסים על בסיס המלצות שלך — רק מה שהלקוח אמר שקיים
    **🔴🔴🔴 חובה בתחילת כל שיחה חדשה:** אם תמונת הנכסים הנוכחית ריקה אבל אתה יודע על נכסים מהיסטוריית השיחות או מהזיכרון — **חובה לקרוא ל-update_portfolio מיד בהודעה הראשונה** כדי לסנכרן. הלקוח כבר סיפר לך על הנכסים קודם, פשוט תעדכן את הפאנל. אל תשאל, אל תגיד "אני מעדכן" — פשוט תקרא לכלי ותמשיך בשיחה רגילה.
...
`;
  ...
  if (assets.length === 0 && (memory.facts?.length > 0 || Object.keys(memory.profile || {}).length > 0)) {
    memoryBlock += `\n\n═══════════════════════════════════════
## ⚠️ תמונת הנכסים ריקה!
הפאנל בצד שמאל של הלקוח ריק. אם יש לך מידע על נכסים מהיסטוריית השיחות או מהזיכרון — **קרא ל-update_portfolio מיד** כדי לסנכרן את כל הנכסים שאתה יודע עליהם. זה קריטי!`;
  }
  if (assets.length > 0) {
